...
const Home = () => {
    ...
    useEffect(() => {
        if (ready) {
            ...
            websocket.onmessage = (event) => {
                ...
                if (msg.event === "pasteurizer-client-closed") {
            ...
            const getOpcuaConn = async () => {
                try {
                    const res = await fetch(
                        `${API_HOST}/v1/pasteurizer/opcuaConnection`,
                        {
                            method: "GET",
                        }
                    );
    ...
    const connectOpcua = async () => {
        setLoader(true)
        try {
            const res = await fetch(`${API_HOST}/v1/pasteurizer/opcuaConnect`, {
                method: "POST",
            });
            if (res.ok) {
    ...
    const disconnectOpcua = async () => {
        setLoader(true)
        try {
            const res = await fetch(`${API_HOST}/v1/pasteurizer/opcuaDisconnect`, {
                method: "POST",
            });
            if (res.ok) {
                dispatch(updateOpcuaConn({ connected: false }));
    ...
    const opcuaConn = useSelector((state) => {
        return state.pasteurizer.opcuaConn;
    });
